...
class Base(SamplerContainer):
	...
	def __init__(self,x,y,hyperparam_kwargs={},derivatives=False,designMatrix=None,priorGroups=None,bounds={},*args,**kwargs):
		...
		if self.f > np.linalg.matrix_rank(self.design_matrix):
			...
			logger.warning("design matrix is of rank %d, but there are %d functions!"%(np.linalg.matrix_rank(self.design_matrix),self.f))
		...
		for i,p in enumerate(self.priorGroups()):
			...
			# 	# else:
			# 	s = "f%d"%f
			# 	fxns.append(Function('%s'%s,self.functionIndex(f),self,f,self.kernels[-1]))

			# # prior = Prior(p,'prior%d'%i,self,self.p,self.derivatives,self.f,self.n,self.x)
			prior = Prior(p,'%d'%i,self,self.p,False,self.f,self.n,self.x)
			self.functionPriors.append(prior)

			samp = prior._samplers[0]

			samplers.extend(prior.samplers())
	...
	def prior_likelihood(self,p,sigma=None,lengthscale=None,prior_ub=None,prior_lb=None):
		...
		mu = np.zeros(self.n)
		cov = self.functionPriors[p].kernel.K(self.x,sigma,lengthscale)
	...
	def samplePrior(self,):
		...
		for p in range(len(self.functionPriors)):
			samples += self.functionPriors[p].sample()
